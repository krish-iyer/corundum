...
async def run_test_shift_in_source_reset(dut):
    ...
    if False:
        ...
        # tb.log.info("trying to received frame")
        # rx_frame = await tb.sink.recv()
        # tb.log.info("received frame")
        # assert rx_frame.tuser
        assert tb.sink.empty()

    tb.log.info("queue: %d", tb.sink.count())
    assert tb.sink.idle()
...
async def run_stress_test(dut, idle_inserter=None, backpressure_inserter=None):
    ...
    if True:
        ...
        while cycles < 100:
            cycles += 1
            if not tb.source.idle() or dut.s_axis_tvalid.value.integer or dut.m_axis_tvalid.value.integer:
                cycles = 0
...
def test_axis_async_fifo(request, data_width, s_clk, m_clk):
